...
state.schools.data <- function(gias_date){
  ...
  state_school_flag <- dplyr::mutate(all_schools_type_status,
                                     phase_type_grouping = dplyr::case_when(.data$phase_of_education_name == "Nursery" ~ "State-funded Nursery",
                                                                            ...
                                                                            (.data$phase_of_education_name %in% c("Secondary", "Middle deemed secondary", "Not applicable", "All-through") & .data$type_of_establishment_name %in% state_funded_list) | .data$type_of_establishment_name == "City technology college" ~ "State-funded Secondary",
                                                                            .data$phase_of_education_name == "16 plus" & .data$type_of_establishment_name %in% state_funded_list ~ "State-funded 16 plus",
