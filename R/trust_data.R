...
#'
#' `academies.trust.data` Imports GIAS Academies Trust Membership History Data from https://www.get-information-schools.service.gov.uk/Downloads.
#' Standardises variable names to be unique and consist only of characters, numbers, and letters and sets all date and numeric variables to relevant classes.
#' Removes duplicates and adds `history_order` variable to show 1 = current trust.
#'
#' @param gias_date the file date of the GIAS cut, defaults to first day of current month if not entered.
...
#' @examples academies.trust.data("2021-06-01", "2021-06-01")
#'
academies.trust.data <- function(gias_date, cut_off_date, urn_link_type = "ofsted"){
  ...
  if(urn_link_type == "ofsted"){
    ...
    if(urn_link_type == "all"){
      urn_links <- all.predecessors.no.splits(gias_date, cut_off_date)
    }
  }
  ...
  trust_history <- dplyr::group_by(linked_academy_info, current_urn, group_id, type_of_establishment_name)
  ...
  trust_list <- dplyr::filter(trust_list,
                              dplyr::n() == 1 | (dplyr::n() > 1 & group_status == "Open"))
  ...
  update_academy_type <- tidyr::pivot_wider(trust_history_ordered,
                                            id_cols = current_urn,
                                            names_from = history_order,
                                            names_prefix = "type_",
                                            values_from = type_of_establishment_name)
  ...
  academy_trust_history <- dplyr::left_join(academy_trust_history,
                                            dplyr::select(trust_list, group_id, group_type),
                                            by = "group_id")
